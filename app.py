...
ALLOWED_EXTENSIONS = {'jpg', 'jpeg', 'png', 'csv', 'txt', 'npy'}
...
def get_scaler_path():
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models/cnn_lstm_model_scaler.pkl')
    if not os.path.exists(path):
        path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models/scaler.pkl')
    return path
...
def process_ecg_image(image_path):
    ...
    def to_base64(image):
        try:
            ...
            print(f"Error encoding image: {encode_err}")
            return ""

    # Generate base64 representations of processing stages
    debug_images = {}
    debug_images['original'] = to_base64(gray)
    debug_images['enhanced'] = to_base64(enhanced)
    debug_images['binary'] = to_base64(binary)
    debug_images['largest_component'] = to_base64(largest_component)

    # Save local files for debugging ONLY when not running on Vercel
    if not IS_VERCEL:
        try:
            ...
            print(f"Error writing local debug images: {write_err}")
    
    # Plot the extracted signal to base64 and optionally save to file
    with plt_lock:
        ...
        try:
            ...
            fig.savefig(buf, format='png')
            buf.seek(0)
            debug_images['extracted_signal'] = base64.b64encode(buf.getvalue()).decode('utf-8')
    ...
    return signal.reshape(-1, 1), debug_images
...
def predict_ecg(processed_data):
    # Try ONNX first (lightweight, preferred for Vercel/production)
    if HAS_ONNX:
        ...
        if os.path.exists(onnx_path):
            try:
                ...
                # Ensure input is float32 and run
                raw_pred = session.run([output_name], {input_name: processed_data.astype(np.float32)})[0]
                return raw_pred[0]
            except Exception as onnx_err:
                print(f"ONNX prediction failed: {onnx_err}")
    
    # Fallback to TensorFlow
    if HAS_TF:
        # Check if the .h5 model file exists
        if not os.path.exists(MODEL_PATH):
            # Create a simple CNN-LSTM model if it doesn't exist (local fallback)
            ...
            try:
                ...
                print(f"Failed to build simple TF model: {build_err}")
        ...
        if os.path.exists(MODEL_PATH):
            try:
                ...
                print(f"TensorFlow prediction failed: {tf_err}")
                
    raise RuntimeError("No model inference engine (ONNX Runtime or TensorFlow) is available, or model files are missing.")
...
def analyze(filename):
    ...
    try:
        ...
        if ext == 'csv':
            ...
            ecg_data, debug_images = process_ecg_image(filepath)
            is_image = True
        else:
        ...
        if processed_data is not None:
            # Predict using prediction helper
            raw_prediction = predict_ecg(processed_data)
            ...
            # Dynamic heart rate and rhythm analysis for all input types
            try:
                signal_flat = ecg_data.flatten()
                signal_norm = (signal_flat - np.mean(signal_flat)) / (np.std(signal_flat) + 1e-10)
                peaks, _ = find_peaks(signal_norm, height=1.0, distance=50)
                
                if len(peaks) > 1:
                    rr_intervals = np.diff(peaks)
                    avg_rr = np.mean(rr_intervals)
                    # Assume 250 Hz sampling rate as baseline
                    heart_rate = 60 / (avg_rr / 250)
                    ...
                    heart_rate = 75.0
                    hr_status = "Normal heart rate (estimated)"
                    rhythm_status = "Regular rhythm"
            except Exception as analysis_err:
                print(f"Error doing signal peak analysis: {analysis_err}")
                heart_rate = 75.0
                hr_status = "Normal heart rate (estimated)"
                rhythm_status = "Regular rhythm"

            # Provide more detailed diagnosis and metadata
            if prediction >= 0.5:
    ...
    if request.method == 'POST':
        ...
        if action == 'upload':
            ...
            if 'model_file' not in request.files:
                flash('No model file part', 'danger')
                return redirect(request.url)
            ...
            if model_file.filename == '':
                flash('No selected model file', 'danger')
                return redirect(request.url)
            ...
            if model_file and model_file.filename.endswith('.h5'):
                ...
                if os.environ.get('VERCEL') == '1':
                    flash('Model uploads are disabled on Vercel (read-only filesystem).', 'warning')
                    return redirect(request.url)
                ...
                try:
                    ...
                    if scaler_file and scaler_file.filename != '':
                        scaler_file.save(get_scaler_path())
                    
                    flash(f'Model "{model_name}" uploaded and activated successfully!', 'success')
                except Exception as e:
                    flash(f'Error uploading model: {str(e)}', 'danger')
            else:
                flash('Invalid model file format. Only .h5 files are supported.', 'danger')
